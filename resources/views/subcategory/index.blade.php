                    ...
                    <div class="col-md-12">
                        <div class="card mb-12">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h3 class="card-title">Sub Category </h3> <a href="{{ route('subcategory.create') }}"
                                    class="btn btn-primary btn-sm"><i class="fa fa-plus-circle"></i> Sub category</a>
                            </div>
                            @if (session('success'))
                                        ...
                                        @foreach ($subCategorys as $subCategory)
                                            <tr class="align-middle">
                                                <td>
                                                    ...
                                                    @php
                                                        $perPage = $subCategorys->perPage(); // Number of items per page
                                                        $currentPage = $subCategorys->currentPage(); // Current page number
                                                ...
                                                <td>
                                                    <a href="{{ route('category.edit',$subCategory->getCategory->id)}}">{{ $subCategory->getCategory->name }}
                                                </td>
                                                <td>
                                                ...
                                                <td style="display: flex;column-gap: 5px;"><a
                                                    ...
                                                    <form action="{{ route('subcategory.destroy', $subCategory->id) }}"
                                                        method="POST" id="submitDeleteForm{{ $subCategory->id }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button data-token="{{ $subCategory->id }}"
