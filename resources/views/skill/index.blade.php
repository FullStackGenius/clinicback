                    ...
                    <div class="col-md-12">
                        <div class="card mb-12">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h3 class="card-title">Skill </h3> <a href="{{ route('skill.create') }}"
                                    class="btn btn-primary btn-sm "><i class="fa fa-plus-circle"></i> Skill</a>
                            </div>
                            @if (session('success'))
                                        ...
                                        @foreach ($skills as $skill)
                                            <tr class="align-middle">
                                                <td>
                                                    ...
                                                    @php
                                                        $perPage = $skills->perPage(); // Number of items per page
                                                        $currentPage = $skills->currentPage(); // Current page number
                                                ...
                                                <td style="display: flex;column-gap: 5px;"><a
                                                    ...
                                                    <form action="{{ route('skill.destroy', $skill->id) }}" method="POST"
                                                        id="submitDeleteForm{{ $skill->id }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button data-token="{{ $skill->id }}"
